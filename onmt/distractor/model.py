...
class DGModel(nn.Module):
    ...
    def __init__(self, encoder, decoder, ans_decoder, multigpu=False):
    ...
    def forward_ans(self, src, ques, ans, tgt,
                sent_lengths, word_lengths, ques_length, ans_length,
                dec_state=None):

        tgt = ans[0][:-1]  # exclude last target from inputs
        ...
        decoder_outputs, dec_state, attns = \
            self.ans_decoder(tgt, word_mem_bank, sent_mem_bank,
                         enc_state if dec_state is None else dec_state,
                         word_lengths, sent_lengths, static_attn)
